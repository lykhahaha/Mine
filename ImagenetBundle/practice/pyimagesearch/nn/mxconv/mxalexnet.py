...
class MxAlexNet:
    ...
    def build(classes):
        ...
        # Block #1: first block Conv -> ReLU -> BN -> Pool layer set
        ...
        pool1 = mx.sym.Pooling(data=bn1_1, kernel=(3, 3), pool_type='max', stride=(2, 2))
        do1 = mx.sym.Dropout(data=pool1, p=0.25)

        # Block #2: second block Conv -> ReLU -> BN -> Pool layer set
        ...
        pool2 = mx.sym.Pooling(data=bn2_1, kernel=(3, 3), pool_type='max', stride=(2, 2))
        do2 = mx.sym.Dropout(data=pool2, p=0.25)

        # Block #3: 3 sets (Conv -> ReLU -> BN) * 3 -> Pool layers set
        ...
        pool3 = mx.sym.Pooling(data=bn3_3, kernel=(3, 3), pool_type='max', stride=(2, 2))
        do3 = mx.sym.Dropout(data=pool3, p=0.25)
        
        # Block #4: first set FC -> ReLU -> BN layer
        ...
        # Block #5: second set FC -> ReLU -> BN layer
