...
val_iter = mx.io.ImageRecordIter(path_imgrec=config.VAL_MX_REC, data_shape=(3, 224, 224), batch_size=batch_size, mean_r=means['R'], mean_g=means['G'], mean_b=means['B'])
...
# compile model
model = mx.model.FeedForward(symbol=model, ctx=[mx.gpu(0)], num_epoch=80, optimizer=opt, initializer=mx.initializer.Xavier(), arg_params=arg_params, aux_params=aux_params, begin_epoch=args['start_epoch'])
