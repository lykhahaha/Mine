...
# python train_googlenet.py --checkpoints checkpoints --prefix googlenet --start-epoch 50
...
ap.add_argument('-p', '--prefix', required=True, help='prefix of filename that mxnet creates')
...
train_iter = mx.io.ImageRecordIter(path_imgrec=config.TRAIN_MX_REC, data_shape=(3, 224, 224), preprocess_threads=config.NUM_DEVICES*2, batch_size=batch_size, rand_crop=True, max_shear_ratio=0.1, rotate=15, rand_mirror=True, mean_r=means['R'], mean_g=means['G'], mean_b=means['B'])
val_iter = mx.io.ImageRecordIter(path_imgrec=config.VAL_MX_REC, data_shape=(3, 224, 224), preprocess_threads=config.NUM_DEVICES*2, batch_size=batch_size, mean_r=means['R'], mean_g=means['G'], mean_b=means['B'])

# initialize optimizer
opt = mx.optimizer.Adam(learning_rate=1e-3, rescale_grad=1./batch_size, wd=2e-4)
...
if args['start_epoch'] <= 0:
    ...
    model = mx.model.FeedForward.load(checkpoint_path, args['start_epoch'])
    arg_params, aux_params = model.arg_params, model.aux_params
    model = model.symbol

model = mx.model.FeedForward(symbol=model, ctx=[mx.gpu(i) for i in range(config.NUM_DEVICES)], num_epoch=90, optimizer=opt, initializer=mx.initializer.Xavier(), arg_params=arg_params, aux_params=aux_params, begin_epoch=args['start_epoch'])
