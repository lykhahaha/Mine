# USAGE
# python test_resnet.py --checkpoints checkpoints --prefix resnet --epoch-number 50
...
ap.add_argument('-p', '--prefix', required=True, default='resnet', help='prefix of filename that mxnet creates')
ap.add_argument('-e', '--epoch-number', type=int, default=0, help='epoch number to evaluate vgg')
...
test_iter = mx.io.ImageRecordIter(path_imgrec=config.TEST_MX_REC, data_shape=(3, 224, 224), preprocess_threads=config.NUM_DEVICES*2, batch_size=config.BATCH_SIZE, mean_r=means['R'], mean_g=means['G'], mean_b=means['B'])
