...
# python train_resnet.py --checkpoints checkpoints --prefix resnet --start-epoch 50
...
opt = mx.optimizer.SGD(learning_rate=1e-1, rescale_grad=1./batch_size, wd=2e-4, momentum=0.9)
...
model = mx.model.FeedForward(symbol=model, ctx=[mx.gpu(i) for i in range(config.NUM_DEVICES)], num_epoch=100, optimizer=opt, initializer=mx.initializer.MSRAPrelu(), arg_params=arg_params, aux_params=aux_params, begin_epoch=args['start_epoch'])
