...
class AgeGenderHelper:
    ...
    def visualize_gender(gender_preds, le):
        ...
        for (i, j) in enumerate(idxs):
            ...
            gender = "Male" if gender == 0 or gender == '0' else "Female"
    ...
    def visualize_video(age_pred, gender_pred, age_le, gender_le, canvas, top_left_pts):
        ...
        gender_label = "Male" if gender_le.classes_[gender_idx] in [0, '0'] else "Female"

        cv2.putText(canvas, f'{age_label}: {age_prob:.2f}', (top_left_pts[0], top_left_pts[1] - 25), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (0, 255, 0), 2)
        cv2.putText(canvas, f'{gender_label}: {gender_prob:.2f}', (top_left_pts[0], top_left_pts[1] - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (0, 255, 0), 2)
