...
ap.add_argument('-v', '--val-nb', type=int, default=0, help='validation number restarted')
...
def training(aug, means_path, train_hdf5_path, val_hdf5_path, fig_path, json_path, label_encoder_path, best_weight_path, checkpoint_path, cross_val=None):
    ...
    sp, mp, pp, iap = SimplePreprocessor(config.IMAGE_SIZE, config.IMAGE_SIZE), MeanPreprocessor(means['R'], means['G'], means['B']), PatchPreprocessor(config.IMAGE_SIZE, config.IMAGE_SIZE), ImageToArrayPreprocessor()
    ...
    if args['start_epoch'] == 0:
        ...
        model = AgeGenderNet.build(config.IMAGE_SIZE, config.IMAGE_SIZE, 3, config.NUM_CLASSES, reg=5e-4)
        ...
        K.set_value(model.optimizer.lr, decay(0))
...
if config.DATASET == 'IOG':
    ...
    for i in range(args['val_nb'], config.NUM_FOLD_PATHS):
