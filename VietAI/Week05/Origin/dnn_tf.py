...
def bat_classification():
    ...
    optimizer = tf.train.MomentumOptimizer(learning_rate=0.001, momentum=0.005)
    
    # build a deep neural network
    # https://www.tensorflow.org/api_docs/python/tf/estimator/DNNClassifier
    #classifier = tf.estimator.DNNClassifier(feature_columns=feature_columns, hidden_units=hidden_layers, n_classes=num_class, optimizer=optimizer, activation_fn=activation)
    classifier = tf.estimator.DNNClassifier(feature_columns=feature_columns, hidden_units=hidden_layers, optimizer=optimizer, n_classes=num_class, activation_fn=activation, model_dir='bat_classification')
    
    # Define the training inputs
    # https://www.tensorflow.org/api_docs/python/tf/estimator/inputs/numpy_input_fn
    #train_input_fn = tf.estimator.inputs.numpy_input_fn(x={"x": train_x}, y=train_y, batch_size=batch_size, num_epochs=1, shuffle=False)
    train_input_fn = tf.estimator.inputs.numpy_input_fn(x={"x": train_x}, y=train_y, num_epochs=None, batch_size=batch_size, shuffle=True)
    ...
    confusion_mat = confusion_matrix(test_y, y_hat)/np.sum(confusion_matrix(test_y, y_hat), axis=1)

    print(confusion_mat)
    print(confusion_mat.flatten()[::num_class+1])
...
def mnist_classification():
    ...
    optimizer = optimizer = tf.train.MomentumOptimizer(learning_rate=0.001, momentum=0.005)

    # build a deep neural network
    classifier = tf.estimator.DNNClassifier(feature_columns=feature_columns, hidden_units=hidden_layers, n_classes=num_class, optimizer=optimizer, activation_fn=activation, model_dir='mnist_classification')
