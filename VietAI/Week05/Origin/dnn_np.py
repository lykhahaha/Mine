...
class NeuralNet(object):
    ...
    def compute_loss(self, y, y_hat):
        ...
        reg_loss = 0.0
        for layer in self.layers:
            reg_loss += 0.5 * self.reg * np.sum(layer.w**2)
...
def minibatch_train(net, train_x, train_y, cfg):
    ...
    for e in range(cfg.num_epoch):
        loss_epoch = 0
        for i in range(int(cfg.num_epoch/cfg.batch_size)):
            train_set_x_batch = train_set_x[i * cfg.batch_size:(i+1)*cfg.batch_size]
            train_set_y_batch = train_set_y[i * cfg.batch_size:(i+1)*cfg.batch_size]
            train_set_y_batch = create_one_hot(train_set_y_batch, net.num_class)
            all_x = net.forward(train_set_x_batch)
            y_hat = all_x[-1]
            loss_epoch += net.compute_loss(train_set_y_batch, y_hat)
            grads = net.backward(train_set_y_batch, all_x)
            net.update_weight_momentum(grads, cfg.learning_rate, cfg.momentum_rate)
        loss_epoch /= int(cfg.num_epoch/cfg.batch_size)
        ...
        print("Epoch %d: loss is %.5f" % (e+1, loss_epoch))
