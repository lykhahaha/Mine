...
from logistic_np import add_one, LogisticClassifier, generate_unit_testcase
...
def test(y_hat, test_y):
    ...
    confusion_mat_decimal = np.zeros((10,10))
    ...
    for row_index in range(confusion_mat.shape[0]):
        confusion_mat_decimal[row_index, :] = confusion_mat[row_index, :] / float(occur[row_index])
    print('Confusion matrix:')
    print(confusion_mat_decimal)
    print('Diagonal values:')
    print(confusion_mat_decimal.flatten()[0::11])
...
if __name__ == "__main__":
    ...
    for e in range(num_epoch):    
        ...
        if all_val_loss and abs(val_loss - all_val_loss[-1]) <= sigma:
            break
